...
import com.plantpurple.emojidom.fragments.FragmentClassicSmileCategory2;
...
public class ClassicTabFragmentAdapter extends FragmentStatePagerAdapter {
	Context context;
	
	ArrayList<FragmentParams> mFragmentParams;

	public ClassicTabFragmentAdapter(Context c, FragmentManager fm, ArrayList<FragmentParams> params) {
		super(fm);
	...
	public Fragment getItem(int position) {
		...
		if (position == 0) {
			fragment = new FragmentClassicFavourites();
			
			Bundle args = new Bundle();
			args.putInt(FragmentParams.CATEGORY, XmlBase.CLASSIC_EMOJI);
			args.putString(FragmentParams.CAPTION, context.getString(R.string.cool_tab_fragment_favourites));
			fragment.setArguments(args);
		} else {
			fragment = new FragmentClassicSmileCategory2();
